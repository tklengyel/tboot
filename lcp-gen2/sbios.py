...
class SBIOS( ElementGui ):
  ...
  def createOrShowPanel(self, wx, listPanel, parent, pdef, statusBar):
    """createOrShowPanel - create the SBIOS Panel"""

    #print("createOrShowSbiosPanel hashAlg=%d, panelCreated == %s" % (self.myHashAlg, self.panelCreated))    # DBGDBG
    ...
    if(hashAlgStr == None):
      print("createOrShowSbiosPanel - invalid myHashAlg=%d" % (self.myHashAlg))
    ...
    self.hashAlgEdit = wx.TextCtrl( self.sbiosPanel, size=(75, -1), value=hashAlgStr )
    self.hashAlgEdit.Enable(False)
    sbiosGridSizer.Add(self.hashAlgEdit, pos=(1,20))
    self.sbiosPanelWidgets.append(self.hashAlgEdit)
  ...
  def hidePanel(self):
    ...
    #print("SBIOS hidePanel - hashAlg=%d, len(Widgets)=%d" % (self.myHashAlg, len(self.sbiosPanelWidgets)))  #DBGDBG
    for i in self.sbiosPanelWidgets:
      i.Hide()

  def showPanel(self):
    """showPanel - show the sbios panel"""
    #print("SBIOS showPanel - hashAlg=%d, len(Widgets)=%d" % (self.myHashAlg, len(self.sbiosPanelWidgets)))  #DBGDBG
  ...
  def setElementToDefaults(self):
    ...
    currentList.ElementDefData[self.myIndex].HashAlg      = DEFINES.TPM_ALG_HASH['SHA256']
  ...
  def onAddButtonClick(self, event):
    ...
    self.setListModified()
    currentList.ElementDefData[self.myIndex].SbiosFiles.append(filename)
  ...
  def writeSbiosDef(self, sbiosDefData, f):
    ...
    print("writeSbiosDef dump, hashAlg=%d"  % (self.myHashAlg))  # DBGDBG
